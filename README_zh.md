...
#### 对比 PostgreSQL 19 SQL/PGQ：查询表层 vs. 专用运行时

SQL:2023 和 PostgreSQL 19 引入了 `CREATE PROPERTY GRAPH`、`GRAPH_TABLE` 和标准图语法。这是 PostgreSQL 中图模式查询的长期标准查询表层，并由 PostgreSQL 的 planner 和 optimizer 支撑。

pgGraph 面向更窄的执行场景。它为反复遍历同一拓扑的工作负载预计算图原生 CSR 存储和可重建构件，适合有深度上限、路径限制、过滤器、租户范围和应用分页的遍历。两者是互补的：未来的适配器可以把符合条件的 PostgreSQL SQL/PGQ 模式映射到 pgGraph 的运行时，而通用 SQL/PGQ 查询继续使用 PostgreSQL 的关系执行路径。
