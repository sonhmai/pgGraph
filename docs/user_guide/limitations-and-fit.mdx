...
| SQL-first applications | The current public API is PostgreSQL SQL functions in the `graph` schema |
...
| Immediate GQL or SQL/PGQ compatibility | A database that implements those query languages today |
...
| GQL and SQL/PGQ | Standards-aligned graph query frontends are planned work, not current public APIs |
| Projection modes | The current runtime is effectively `csr_readonly`; a future `mutable_overlay` mode is planned but not implemented |
