...
| GQL and openCypher | `graph.gql()` supports the documented read subset, including node-only `MATCH`, top-level single-relationship `OPTIONAL MATCH`, single-hop path variables with optional named elements and concrete labels/types, projection-stage `WITH`, bounded `DISTINCT`, `RETURN` aggregates, JSONB list/map property paths, bounded relationship path values and path functions, single-node mapped `CREATE`, single-node mapped property `SET`, mapped property `REMOVE`, single-edge mapped `DELETE`, single-node `DETACH DELETE` over registered edge-row relationships, and single-node mapped `MERGE`; `graph.cypher()` accepts the overlapping openCypher-shaped subset through the same planner and explicitly does not claim full openCypher compatibility; multi-segment wildcard paths, wildcard path property predicates, and broader writes are future work |
