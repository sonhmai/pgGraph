...
| Tag | Description |
|---|---|
| `0.1.5` | Pinned reproducible tag (PostgreSQL 17) — always resolves to the same image digest |
| `latest` | Moving tag that points to the most recent stable PostgreSQL 17 image |
| `pg14-0.1.5` through `pg18-0.1.5` | Versioned image for a specific PostgreSQL major |
