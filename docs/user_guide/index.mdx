...
including a documented read-only GQL-compatible subset through `graph.gql()`.
SQL/PGQ adapters, GQL writes, and broader graph-language coverage remain
future work.
</Callout>
...
| Read-only GQL | A documented GQL-compatible graph-pattern subset over registered tables and edges, returned as JSONB rows |
