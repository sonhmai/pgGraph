...
## v0.1.7: Stable Bidirectional Relationship Results

`v0.1.7` fixes how GQL returns relationship values for bidirectional edges.
Edges can still be traversed in either direction, but result rows now present
relationship endpoints in the direction registered in the graph schema. This
keeps edge records stable for applications that inspect `_start`, `_end`, path
values, or `relationships(path)`.
...
- Applied the same stable endpoint orientation to path values and
  `relationships(path)`.
- Updated the persisted `.pggraph` artifact format to store the edge-direction
  metadata needed for stable relationship projection.
- Kept PostgreSQL source tables authoritative; persisted graph files remain
...
The PostgreSQL 17 release gate passed, including unit tests, pgrx SQL tests,
package validation, install smoke, metadata audit, SQLSTATE/ACL checks,
backup/restore, concurrency stress, playground query validation, pgbench sync
stress, Docker smoke, and disposable crash recovery checks.
